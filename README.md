...
- `packages/leisaac/`: AICapstone-specific `leisaac` extension package only, not a full upstream copy
...
The root `pyproject.toml` wires `umi` and `leisaac` through `[tool.uv.workspace]` and `[tool.uv.sources]`, while each workspace member keeps its own `pyproject.toml` under `packages/`. The `leisaac` workspace member is intentionally limited to the extensions being developed in this repo.
